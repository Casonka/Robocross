...
#include <FilConfig.h>
    ...
    */
    ...
    #define DMA_init(dma,event,priority,view,address_1,address_2,size)   {      \
                    ...
                    conf_dma_psize(dma,event,size);                             \
                    conf_dma_msize(dma,event,size);                             \
                    conf_dma_pl(dma,event,0x3);                                 \
                    conf_dma_tcie(dma,event,0x1);                               \
                    conf_dma_ndtr(dma,event,0x9);                               \
        ...
        */
        #define DMA_ADC_init(event,target_addr,receiver_addr)     DMA_init(dma2,event,0x3,0x0,target_addr,receiver_addr,0x1)
    ...
    #define dma_ndtr(ndtr)          ((uint32_t)(ndtr))
    ...
    #define clear_dma_ndtr(dma,ndtr)                            (* (uint32_t *)(dma_id(dma)) + (0x14 + check_stream(default_s,event))) &= (~dma_ndtr(0xFFFFFFFF))
    ...
    #define conf_dma_ndtr(dma,event,ndtr)          {* ((uint32_t *)(dma_id(dma)) + (0x14 + check_stream(default_s,event))) = \
                                                    ((* ((uint32_t *)(dma_id(dma)) + (0x14 + check_stream(default_s,event))))&(~dma_ndtr(0xFFFFFFFF)))|dma_ndtr(ndtr);}
